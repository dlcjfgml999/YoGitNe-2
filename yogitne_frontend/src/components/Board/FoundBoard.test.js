...
import FoundBoard from './FoundBoard';

configure({ adapter: new Adapter() });
const mockStore = configureMockStore();

describe('FOUNDBOARD TEST', () => {
  let wrapper, store;
  ...
  beforeEach(() => {
    ...
    wrapper = shallow(<FoundBoard store={store} />);
  });
  it('Initial State To Props', () => {
    expect(wrapper.props().Articles).toEqual([1, 2, 3, 4, 5]);
  });
  it('Call componentDidMount Check', () => {
    const spy = jest.spyOn(FoundBoard.prototype, 'componentDidMount');
    const wrapper2 = shallow(<FoundBoard store={store} />);
  ...
  it('Call componentDidMount with mount', () => {
    ...
    const mapper = mount(<Provider store={mstore}><Router><FoundBoard store={mstore} /></Router></Provider>);
  });
