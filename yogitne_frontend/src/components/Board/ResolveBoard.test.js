...
describe('RESOLVEBOARD TEST', () => {
  ...
  beforeEach(() => {
    ...
    wrapper = shallow(<ResolveBoard store={store} />);
  });
  it('Initial State To Props', () => {
    expect(wrapper.props().Articles).toEqual([1, 2, 3, 4, 5]);
  });
  it('Call componentDidMount Check', () => {
    const spy = jest.spyOn(ResolveBoard.prototype, 'componentDidMount');
    const wrapper2 = shallow(<ResolveBoard store={store} />);
  ...
  it('Call componentDidMount with mount', () => {
    ...
    const mapper = mount(<Provider store={mstore}><Router><ResolveBoard store={mstore} /></Router></Provider>);
  });
